...
const leg = (over: Partial<AirportLeg> = {}): AirportLeg => ({
  kind: "rail", serviceName: "AREX-E112",
...
const render = (legs: AirportLeg[]) => renderToStaticMarkup(createElement(DayGatewayInfo, {
  legs, stationName: (id: string) => `${id}역`, date: "2026-08-12",
...
describe("내용", () => {
  ...
  it("버스 구간은 버스로 적는다", () => {
    expect(render([leg({ kind: "bus", serviceName: "공항버스 6001" })])).toContain("공항버스");
  });
