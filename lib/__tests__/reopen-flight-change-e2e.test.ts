...
class Screen {
  ...
  async startFrom(trip: PlanRequest) {
    const inputs = tripInputsFromConstraints(trip);
    ...
    this.candidates = (await getCandidatePlaces(SELECTION)).candidates;
    this.selected = new Set(initialCandidateIds(this.candidates));
  }
...
/** 저장본 하나와, 그와 **다른** 일정을 화면에 띄운 상태를 만든다 */
async function screenWithSavedRecord() {
  const screen = new Screen();
  await screen.startFrom(FIRST_TRIP);
  await screen.plan();
  ...
  // 저장 뒤 다른 조건으로 계산해 둔다 — 화면의 추천과 저장본이 같으면
  // "다시 열면 저장본을 보여준다"를 증명하지 못한다
...
describe("#6 다시 연 일정에서 항공편 변경과 재계산", () => {
  ...
  it("다시 열면 항공 입력이 저장 당시 값으로 돌아온다", async () => {
    const { screen, saved } = await screenWithSavedRecord();
    // 지금 화면 입력은 저장 당시와 다르다
    expect(at(screen.currentConstraints()?.arrivalAt)).not.toBe(at(saved.constraints.arrivalAt));

    await screen.reopen(saved);

    // 입력이 안 돌아오면 이후 재계산이 엉뚱한 조건을 보낸다
    expect(at(screen.currentConstraints()?.arrivalAt)).toBe(at(saved.constraints.arrivalAt));
    expect(at(screen.currentConstraints()?.airportReadyAt))
      .toBe(at(saved.constraints.airportReadyAt));
...
describe("하네스가 화면과 같은 진입점을 본다", () => {
  ...
  it.each(["setArrival", "setDeparture", "setAirportReady", "setAirportDeadline"])(
    "재열람이 %s 로 입력을 복원한다",
    (setter) => {
