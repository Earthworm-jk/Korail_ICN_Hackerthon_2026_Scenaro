...
describe("재질문 조각 잇기", () => {
  ...
  describe("날짜 답변만 이어 붙인다", () => {
    ...
    /**
     * PR #175 리뷰 6회차 — "며칠째로 옮길까요?"에 존댓말로 답하는 것이 가장 흔한 대답이다.
     * 여기서 끊기면 이 PR이 고치려는 흐름이 그대로 다시 막힌다.
     */
    it("존댓말로 답해도 읽는다", () => {
      for (const [text, day] of [
        ["둘째 날이요", 2], ["둘째 날입니다", 2], ["둘째 날이에요", 2],
        ["셋째 날예요", 3], ["2일차입니다", 2], ["둘째 날이야", 2],
        ["2일차로", 2], ["둘째 날에서", 2], ["day 2 please", 2],
      ] as const) {
        expect(dayOnlyAnswer(text), text).toBe(day);
      }
    });
