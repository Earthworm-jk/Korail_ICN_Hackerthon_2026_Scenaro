import "server-only";
/**
 * 인천공항 여객편 운항 현황 실호출 어댑터 (API_SPEC 2.1, REQ-DATA-003)
 *
 * - 유일한 런타임 실호출. 5초 초과 또는 오류 시 호출부(Action)가 스냅샷으로 폴백한다.
 * - 이 API는 편명·날짜 필터 파라미터가 없어(2026-08-08 실측) 전체 목록(약 2MB, 3일 구간)을
 *   1회 수신 후 로컬에서 편명을 찾는다. 결과는 방향별로 5분 캐시해 재조회를 막는다.
...
const ENDPOINT = "https://apis.data.go.kr/B551177/StatusOfPassengerFlightsDSOdp";
const OPERATIONS = {
  arrival: "getPassengerArrivalsDSOdp",
  departure: "getPassengerDeparturesDSOdp",
} as const;
const TIMEOUT_MS = 5_000; // API_SPEC 2.1 — 이 시간 안에 못 받으면 스냅샷 폴백
const CACHE_TTL_MS = 5 * 60_000;
const MAX_ROWS = 10_000; // 전체 구간(실측 약 7,600건)을 한 페이지로

export type LiveFlightRecord = {
  flightId: string;
  scheduleDateTime: string; // yyyyMMddHHmm (KST)
  estimatedDateTime?: string;
  remark?: string; // 도착·착륙·출발·지연·결항 등
  terminalid?: string; // P01·P02·P03
  codeshare?: string; // Master | Slave
};
...
export function parseAirportDateTime(value: string | undefined): string | null {
...
export function terminalLabel(terminalid: string | undefined): string | undefined {
 ...
 * 같은 편명이 여러 날짜에 있으면 Master 우선 → 예정 시각 오름차순 첫 건 (결정적).
 */
...
async function fetchRecords(
  ...
  try {
    ...
    for (const variant of serviceKeyVariants(key)) {
      const url = `${ENDPOINT}/${OPERATIONS[direction]}?serviceKey=${variant}&type=json&numOfRows=${MAX_ROWS}&pageNo=1`;
      try {
        ...
        cache.set(direction, { at: now(), records });
        return records;
      } catch (error) {
...
/** 실호출 1건 — 실패(타임아웃·네트워크·키)는 throw하고 호출부가 스냅샷으로 폴백한다 */
export async function lookupLiveFlight(
  flightNo: string,
  direction: "arrival" | "departure",
  deps: Deps = {},
): Promise<LiveLookupResult> {
  const records = await fetchRecords(direction, deps);
