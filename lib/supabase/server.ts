 ...
 *   NFR-DEMO-001과 같은 원칙). publishable/anon 키는 공개 가능한 클라이언트 키이며 RLS가 접근을 통제한다.
 */
import "server-only";
 ...
 * PR #74 리뷰 1: 신규 프로젝트 키 체계인 publishable key(sb_publishable_...)를 우선하고,
 * legacy NEXT_PUBLIC_SUPABASE_ANON_KEY는 호환용으로만 읽는다. */
export function supabaseConfig(
  source: Record<string, string | undefined> = process.env,
): SupabaseConfig | null {
  const url = source.NEXT_PUBLIC_SUPABASE_URL?.trim();
  const key = source.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY?.trim()
