...
대표 촬영지 여부는 검수된 작품–장소 관계의 `representativeness`만 사용한다. OpenAI는 배포 전
파이프라인에서 검증된 근거를 구조화하고 등급을 제안할 수 있지만, 제안 스냅샷 자체는 런타임 입력이
아니다. 사람이 승인해 관계 시드에 고정한 `iconic`만 사용한다. 모델 기억으로 런타임에 추측하지
않으며, 사용자가 해당 장소를 제외하면 대표성보다 제외가 우선한다.
