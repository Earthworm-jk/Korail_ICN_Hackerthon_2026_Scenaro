...
이 문서는 디자인 담당자가 추가 기획 없이 씬나로 최종 MVP 화면과 주요 예외 상태의 시안을 그릴 수 있도록 화면 흐름, 정보 위계, 컴포넌트, 상태, 문구와 상호작용을 고정한다.

와이어프레임은 다음 질문에 답해야 한다.

1. 사용자는 항공 일정과 K-콘텐츠 취향을 어떤 순서로 입력하는가?
2. 후보 촬영지 중 방문하고 싶은 장소를 어떻게 선택하는가?
3. 시스템이 만든 일정과 시간표의 근거를 어떻게 이해하는가?
4. 조건 변경 후 갱신된 일정을 어떻게 확인하는가?
...
- **선택권:** 시스템은 추천하지만 방문 후보와 대체 시간표는 사용자가 고른다.
- **설명 가능성:** 추천·제외·변경·실패 이유를 숨기지 않는다.

## 3. 범위와 우선순위

### 3.1 최종 MVP 시안에 반드시 포함

- 항공편·여행기간 입력
- 배우·작품 통합 검색
- 작품·촬영지 후보 확인과 일반 장소 선택
- 조건 요약과 일정 생성
- 일자별 철도 기반 일정 결과
- 촬영지 제외, 항공편 시각 변경
- 재계산 후 갱신된 일정 표시
...
- 결과 화면 중심 영어 전환
- 저장 시 로그인, 저장 성공·실패, 내 일정·다시 열기

와이어프레임에는 대안 시간표의 접힌 상태와 펼친 상태를 모두 그린다.
...
배우만, 작품만, 배우+작품 조합을 모두 허용한다. 배우와 작품은 모두 복수 선택할 수 있다. 선택 배우와 무관한 작품도 함께 선택할 수 있으며, 후보 촬영지는 선택 배우 관련 장소와 선택 작품 관련 장소의 합집합으로 만들고 중복 장소를 제거한다.
...
- 선택 영역: 배우 복수, 작품 복수 chip

### 규칙

- 배우 결과에는 이름, 대표작, 검증 작품 수 표시.
- 작품 결과에는 제목, 연도, 배우, 검증 촬영지 수 표시.
- 배우를 선택하면 선택한 모든 배우의 작품을 중복 없이 바로 노출한다.
- 선택 배우들과 관련 없는 작품도 독립적으로 검색·추가할 수 있다.
...
- 모든 연결 장소는 일반 후보로 보인다.
- 운영시간 미확인 장소는 `확인 필요` 배지로 표시하되 후보에서 숨기지 않는다.
- 선택 장소가 많아지면 `선택한 장소가 많아 모두 방문하지 못할 수 있어요.` 안내를 표시한다.
...
- 장소 카드의 `제외` 클릭
- 확인 팝오버: `주문진 방사제를 제외하고 전체 일정을 다시 계산할까요?`
- Primary: `제외하고 재계산`
- 취소 가능
...
결과 상단에 재계산 완료 배너를 배치하고 갱신된 일정 전체를 표시한다.

예시:

> 새 조건으로 일정을 다시 계산했습니다.

이전 값과 새 값을 나란히 비교하거나 빠진 항목에 취소선을 표시하지 않는다. 사용자가 지금 따라야 할
최신 DAY 카드와 열차 시각을 우선해서 보여준다.
...
| `DEPARTURE_DEADLINE_EXCEEDED` | `출국편에 맞춰 돌아올 수 없어요` | `출국 안전 여유 120분을 지키면 마지막 일정이 시간 안에 들어오지 않습니다.` | `선택 장소 줄이기` |

개발용 사유 코드 문자열을 그대로 사용자에게 노출하지 않는다.

`ACTIVITY_WINDOW_MISMATCH`는 최초 생성 불가 사유가 아니라 장소 카드의 운영시간 확인 경고로
표시한다. 운영시간 밖이라는 이유만으로 장소를 자동 제외하지 않는다.
...
| `D-09 Edit controls` | WF-09의 편집 2종 |
...
- Badge: verified, estimate, needs verification, selected, snapshot
- Place card: default, selected, unavailable, needs verification
...
- Modal/popover: exclude, edit flight
